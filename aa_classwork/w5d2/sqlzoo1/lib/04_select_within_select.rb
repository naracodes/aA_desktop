...
def population_constraint
  ...
  execute(<<-SQL)
    ...
    FROM countries
    WHERE population BETWEEN (SELECT population FROM countries WHERE name = 'Canada') AND (SELECT population FROM countries WHERE name = 'Poland') AND name NOT IN ('Canada', 'Poland')
  SQL
end
